...
@section('content-wrapper')
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ URL::to('data-riwayat-kasus/' . $dataRiwayatKasus->id_kasus) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kode_diagnosa') is-invalid @enderror"
                            ...
                            value="{{ old('kode_diagnosa', $dataRiwayatKasus->kode_kasus) }}" autofocus>
                        @error('kode_diagnosa')
                    ...
                    <div class="mb-3">
                        ...
                        <div id="containerSolusi">
                            <button class="btn btn-gradient-custom mb-3" type="button" id="btnTambahDaftarGejala"
                            ...
                            @if ($errors->has('daftarGejala'))
                                ...
                                @if (old('daftarGejala'))
                                    @for ($i = 0; $i < count(old('daftarGejala', json_decode($dataRiwayatKasus->array_gejala))); $i++)
                                        <div class="row" id="daftarGejalaContainer-{{ $i }}">
                                            <div class="col-sm-11">
                                                <select class="form-select mb-2" name="daftarGejala[]"
                                                    id="daftarGejala-{{ $i }}">
                                                    @foreach ($dataGejala as $gejala)
                                                        @if (old('daftarGejala.' . $i, json_decode($dataRiwayatKasus->array_gejala)[$i]) == $gejala->kode_gejala)
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('hasil_diagnosa') is-invalid @enderror" name="hasil_diagnosa"
                            ...
                            @foreach ($dataPenyakit as $penyakit)
                                @if (old('hasil_diagnosa', $dataRiwayatKasus->hasil_diagnosa) == $penyakit->nama_penyakit)
